...
cyd <- cyData(counts, markerData=marker.data, medianIntensities=medians, cellIntensities=cell.int)
expect_error(cyData(counts[1:10,], markerData=marker.data, medianIntensities=medians, cellIntensities=cell.int),
             "'medianIntensities' and 'object' must have same number of rows")
expect_error(cyData(counts, markerData=marker.data[1:10,], medianIntensities=medians, cellIntensities=cell.int),
             "'markerData' and 'cellIntensities' must have same number of rows")
expect_error(cyData(counts, markerData=marker.data, medianIntensities=medians[1:10,], cellIntensities=cell.int),
             "'medianIntensities' and 'object' must have same number of rows")
expect_error(cyData(counts, markerData=marker.data, medianIntensities=medians[,1:10], cellIntensities=cell.int),
             "number of rows in 'markerData' and columns in 'medianIntensities' must be equal")
expect_error(cyData(counts, markerData=marker.data, medianIntensities=medians[,1:10], cellIntensities=cell.int[1:10,]),
             "'markerData' and 'cellIntensities' must have same number of rows")
...
cyd <- cyData(counts, metadata=list("YAY"), markerData=marker.data, medianIntensities=medians, cellIntensities=cell.int)
...
cyd2 <- cyData(counts2, markerData=marker.data, medianIntensities=medians2, cellIntensities=cell.int)
cyd3 <- cyData(counts2, markerData=marker.data[rev(seq_len(nrow(marker.data))),],
               medianIntensities=medians2, cellIntensities=cell.int)
...
cyd.minimal <- cyData(assays=matrix(0, 0, 10), markerData=marker.data, cellIntensities=cell.int)
expect_equal(rbind(cyd, cyd.minimal), cyd)
cyd.minimal <- cyData(assays=matrix(0, nrow(medians), 0), medianIntensities=medians, 
    markerData=marker.data, cellIntensities=cell.int)
